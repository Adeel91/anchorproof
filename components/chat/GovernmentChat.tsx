  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      role: 'assistant',
      content:
        "Hello! I'm your government compliance AI assistant. I can help with FOIA, FISMA, FedRAMP, and public records compliance. All conversations are cryptographically verified and tamper-proof. How can I assist you today?",
    },
  ]);
    ...
    <div className="relative w-full mx-auto px-2 sm:px-0">
      <div className="bg-slate-900/60 border border-slate-700/50 rounded-xl sm:rounded-2xl overflow-hidden shadow-2xl shadow-purple-500/5">
        <div className="bg-gradient-to-r from-slate-900 via-slate-900/95 to-slate-900 px-3 sm:px-6 py-3 sm:py-4 border-b border-slate-700/50">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
            <div className="flex items-center gap-2 sm:gap-3">
              <div className="w-8 h-8 sm:w-11 sm:h-11 rounded-lg sm:rounded-xl bg-gradient-to-br from-purple-500 to-pink-600 flex items-center justify-center shadow-lg shadow-purple-500/20 flex-shrink-0">
                <Landmark className="w-3.5 h-3.5 sm:w-5 sm:h-5 text-white" />
              </div>
              <div>
                <div className="flex items-center gap-1.5 sm:gap-2">
                  ...
                  <span className="px-1.5 sm:px-2 py-0.5 bg-purple-500/20 text-purple-400 text-[6px] sm:text-[10px] font-mono font-semibold rounded-full border border-purple-500/30 whitespace-nowrap">
                    LIVE DEMO
                  </span>
                </div>
        ...
        <div className="hidden xs:flex bg-slate-800/30 px-3 sm:px-6 py-1.5 sm:py-2 border-b border-slate-700/30 flex-wrap items-center gap-2 sm:gap-4 text-[8px] sm:text-[10px] text-slate-500">
          <span className="flex items-center gap-1 sm:gap-1.5">
            ...
            <span className="hidden sm:inline">FOIA & FedRAMP Compliant</span>
            <span className="sm:hidden">FOIA/FedRAMP</span>
          </span>
          <span className="w-px h-2 sm:h-3 bg-slate-700" />
          <span className="flex items-center gap-1 sm:gap-1.5">
              ...
              >
                <div
                  className={`max-w-[90%] xs:max-w-[85%] sm:max-w-[80%] rounded-xl sm:rounded-2xl px-3 sm:px-4 py-2 sm:py-3 ${
                    message.role === 'user'
                      ? 'bg-gradient-to-br from-purple-600 to-pink-700 text-white shadow-lg shadow-purple-500/20'
                      : 'bg-slate-800/80 text-slate-200 border border-slate-700/50'
                  }`}
                >
                  <div className="flex items-center gap-1.5 sm:gap-2 mb-0.5 sm:mb-1">
                    {message.role === 'assistant' ? (
                      <Building2 className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-purple-400" />
                    ) : (
                      <User className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-purple-300" />
                    )}
        ...
        <div className="bg-slate-900/80 px-2 sm:px-4 py-2 sm:py-3 border-t border-slate-700/50">
          <div className="flex gap-1.5 sm:gap-2">
            <textarea
              ...
              rows={1}
              className="flex-1 bg-slate-800 border border-slate-700 rounded-lg sm:rounded-xl px-2.5 sm:px-4 py-2 sm:py-3 text-white text-xs sm:text-sm resize-none focus:outline-none focus:border-purple-400 focus:ring-1 focus:ring-purple-400/30 transition-all placeholder:text-slate-500 min-h-[36px] sm:min-h-[42px] max-h-[60px] sm:max-h-[80px]"
            ...
            <Button
              ...
              className="px-3 sm:px-6 h-[36px] sm:h-[46px] flex-shrink-0 shadow-lg shadow-purple-500/20 text-xs sm:text-sm"
            >
          ...
          <div className="flex flex-wrap items-center justify-center gap-1.5 sm:gap-3 mt-1.5 sm:mt-2 text-[7px] sm:text-[10px] text-slate-500">
            <span className="flex items-center gap-0.5 sm:gap-1">
              <Lock className="w-2 h-2 sm:w-3 sm:h-3 text-purple-400" />
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 sm:gap-4">
            <div className="flex-1 w-full sm:w-auto">
              <div className="flex items-center gap-1.5 sm:gap-2 mb-0.5 sm:mb-1">
                ...
                <span className="text-[8px] sm:text-[10px] font-semibold text-purple-400 uppercase tracking-wider">
                  FOIA Verification
                </span>
              </div>
              <p className="text-[10px] sm:text-sm text-slate-300 leading-relaxed">
                Save as immutable record on Walrus and Sui blockchain.
                <span className="text-purple-400 font-medium ml-1 hidden xs:inline">
                  Tamper-proof evidence for government compliance.
                </span>
              </p>
            </div>
            <Button
              ...
              disabled={isSaving}
              className="px-4 sm:px-7 py-1.5 sm:py-3 text-[10px] sm:text-sm font-semibold w-full sm:w-auto shadow-lg shadow-purple-500/20"
            >
              {isSaving ? (
                <>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-md flex items-center justify-center z-50 p-3 sm:p-4">
          <div className="bg-gradient-to-b from-slate-900 to-slate-950 border border-slate-700 rounded-2xl p-6 sm:p-8 max-w-2xl w-full shadow-2xl shadow-purple-500/10 max-h-[95vh] sm:max-h-[90vh] overflow-y-auto animate-in zoom-in-95 duration-300 mx-2 sm:mx-0">
