...
const getReportId = (
  reportData: ComplianceReportPDFProps['reportData']
): string => {
  const { generatedAt, tenantName, conversations } = reportData;
  const timestamp = new Date(generatedAt).getTime();
  const convCount = conversations?.length || 0;
  return `AP-${timestamp}-${tenantName.slice(0, 4)}-${convCount}`;
};
...
export function ComplianceReportPDF({ reportData }: ComplianceReportPDFProps) {
  ...
  const title =
    reportTitle ||
    (isSingleReport ? 'Conversation Report' : 'Compliance Report');
  ...
  const reportId = getReportId(reportData);
    ...
    <Document>
      <Page size="A4" style={styles.page}>
        <View style={styles.coverPage}>
          ...
          <View style={{ marginTop: 40 }}>
            ...
            <Text style={styles.metaText}>
              <Text style={styles.metaTextBold}>Generated:</Text>{' '}
              {new Date(generatedAt).toLocaleString()}
            </Text>
            <Text style={styles.metaText}>
              <Text style={styles.metaTextBold}>Generated By:</Text>{' '}
              {generatedBy}
            </Text>
            <Text style={styles.metaText}>
              <Text style={styles.metaTextBold}>Report ID:</Text> {reportId}
            </Text>
            {isSingleReport && conversations[0] && (
              <Text style={styles.metaText}>
                <Text style={styles.metaTextBold}>Conversation:</Text>{' '}
          ...
          <Text style={styles.footerNote}>
            This report serves as court-admissible evidence of conversation
            integrity. All data is cryptographically verified and stored on the
            Sui blockchain.
          </Text>
        </View>
      </Page>
      ...
      <Page size="A4" style={styles.page}>
        ...
        <View style={styles.summaryGrid}>
          ...
          <View style={styles.summaryCard}>
            ...
            <Text
              style={[
                styles.summaryValue,
                { color: hasTampered ? '#ef4444' : '#10b981' },
              ]}
            >
        ...
        <View style={styles.cryptoProof}>
          ...
          <Text style={styles.cryptoItem}>
            • All conversations stored on Walrus decentralized storage
          </Text>
          <Text style={styles.cryptoItem}>
            • Each conversation has a unique blobId and blockchain transaction
            hash
          </Text>
          <Text style={styles.cryptoItem}>
            • Messages are cryptographically signed and verified
          </Text>
          <Text style={styles.cryptoItem}>
        ...
        <View
          style={[
            ...
            isCompliant ? styles.integrityPass : styles.integrityFail,
          ]}
        >
          <Text>
            ...
            <View style={styles.conversationBlock}>
              <View style={styles.conversationHeader}>
                <Text style={styles.conversationId}>
                  {isSingleReport ? 'Conversation' : `#${index + 1} —`}{' '}
                  {conv.conversationId?.slice(0, 24)}...
                </Text>
                <Text
                  style={[
              ...
              <Link
                src={`https://walruscan.com/${activeNetwork}/blob/${conv.blobId}`}
              >
              ...
              {qrCodes[conv.blobId] && (
                <View style={styles.qrSection}>
                  ...
                  <View>
                    ...
                    <Text
                      style={[styles.qrText, { fontSize: 7, color: '#9ca3af' }]}
                    >
                      {`https://walruscan.com/${activeNetwork}/blob/${conv.blobId}`}
                    </Text>
                  </View>
                </View>
              )}
              ...
              <View style={styles.messagesSection}>
                ...
                {messageCount > 0 ? (
                  conv.messages.map((msg, idx: number) => (
                    <View
                      key={idx}
                      style={[
                        styles.message,
      ...
      <Page size="A4" style={styles.page}>
        ...
        <View style={styles.cryptoProof}>
          ...
          <Text style={styles.cryptoItem}>
            This report contains {summary.totalConversations} conversations
          </Text>
          <Text style={styles.cryptoItem}>
            {summary.verifiedCount} verified • {summary.tamperedCount} tampered
          </Text>
          <Text style={styles.cryptoItem}>
            Integrity Rate: {summary.integrityRate}%
          </Text>
        </View>

        <View style={{ marginTop: 20 }}>
          <Text style={{ fontSize: 11, fontWeight: 'bold', marginBottom: 8 }}>
            All conversations are stored on:
          </Text>
