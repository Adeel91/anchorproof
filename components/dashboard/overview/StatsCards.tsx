    ...
    return (
      <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        {[1, 2, 3, 4].map((i) => (
          <div
            key={i}
            className="bg-slate-900/50 border border-slate-800/50 rounded-xl p-4 sm:p-6 animate-pulse"
          >
        ...
        return (
          ...
          >
            <div className="flex items-center justify-between mb-2 sm:mb-4">
              <span className="text-[10px] sm:text-sm font-medium text-slate-400 tracking-wide uppercase">
                ...
                <span className="xs:hidden">
                  {card.label === 'Total Conversations'
                    ? 'Conversations'
                    : card.label === 'Verified Records'
                      ? 'Verified'
                      : card.label === 'Pending Verification'
                        ? 'Pending'
                        : 'Messages'}
                </span>
              </span>
            ...
            <div className="flex items-end justify-between">
              <div
                className={`text-xl sm:text-3xl font-bold bg-clip-text text-transparent bg-gradient-to-r ${card.color}`}
              >
                {card.value}
              </div>
              {card.trend && (
                <div
                  className={`flex items-center gap-0.5 sm:gap-1 text-[9px] sm:text-xs font-medium ${
