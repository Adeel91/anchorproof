...
export function SystemStatus() {
  ...
  return (
    <div className={`bg-gradient-to-r ${isHealthy ? 'from-emerald-500/10 via-cyan-500/10 to-indigo-500/10 border-emerald-500/20' : 'from-amber-500/10 via-red-500/10 to-orange-500/10 border-amber-500/20'} border rounded-xl p-3 sm:p-4 transition-all duration-500`}>
      ...
      <div className="flex flex-col gap-2 sm:hidden">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <div className={`w-1.5 h-1.5 rounded-full ${isHealthy ? 'bg-emerald-400' : 'bg-amber-400'} animate-pulse`} />
            <span className={`text-[10px] font-mono ${isHealthy ? 'text-emerald-400' : 'text-amber-400'}`}>
      ...
      <div className="hidden sm:flex flex-wrap items-center justify-between gap-2 md:gap-4">
        ...
        <div className="flex flex-wrap items-center gap-2 md:gap-3">
          <div className="flex items-center gap-1.5 md:gap-2">
            <div className={`w-1.5 h-1.5 md:w-2 md:h-2 rounded-full ${isHealthy ? 'bg-emerald-400' : 'bg-amber-400'} animate-pulse`} />
            <span className={`text-[10px] md:text-xs font-mono ${isHealthy ? 'text-emerald-400' : 'text-amber-400'}`}>
              {isHealthy ? 'SYSTEM OPERATIONAL' : 'REVIEW REQUIRED'}
            </span>
          </div>
          ...
          <div className="flex items-center gap-1.5 md:gap-2 text-[10px] md:text-xs text-slate-400">
            ...
            <span className="text-white font-mono truncate max-w-[100px] md:max-w-[150px]">{tenantName}</span>
            <span className="hidden lg:inline text-slate-600">|</span>
            <span className="hidden lg:inline">Tier:</span>
            <span className="hidden lg:inline text-white font-mono">{subscriptionTier}</span>
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-1.5 md:gap-2 lg:gap-4">
          ...
          <div className="flex items-center gap-1 md:gap-1.5 text-[10px] md:text-xs text-slate-400">
            <Shield className={`w-3 h-3 md:w-3.5 md:h-3.5 ${verificationRate >= 80 ? 'text-emerald-400' : verificationRate >= 50 ? 'text-amber-400' : 'text-red-400'}`} />
