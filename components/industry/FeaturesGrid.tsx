// components/industry/FeaturesGrid.tsx
'use client';
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center max-w-3xl mx-auto mb-12 sm:mb-16">
          ...
          <h2 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-bold text-white">
            Built for{' '}
            <span className={`bg-clip-text text-transparent bg-gradient-to-r from-${gradientColor}-400 to-${gradientColor === 'amber' ? 'cyan' : gradientColor === 'emerald' ? 'teal' : gradientColor === 'indigo' ? 'purple' : 'indigo'}-400`}>
              {highlightedText}
            </span>
          </h2>
                ...
                <ul className="space-y-2 sm:space-y-3">
                  {feature.items.map((item, j) => (
                    <li key={j} className="flex items-start gap-2 text-xs sm:text-sm text-slate-300">
                      <span className={`w-1.5 h-1.5 rounded-full bg-${gradientColor}-400 mt-1.5 flex-shrink-0`} />
                      {item}
                    </li>
                  ))}
                </ul>
